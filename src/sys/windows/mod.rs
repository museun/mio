...
unsafe fn no_notify_on_instant_completion(handle: winapi::um::winnt::HANDLE) -> io::Result<()> {
    ...
    const FILE_SKIP_COMPLETION_PORT_ON_SUCCESS: winapi::shared::ntdef::UCHAR = 1;
    const FILE_SKIP_SET_EVENT_ON_HANDLE: winapi::shared::ntdef::UCHAR = 2;
    ...
    if (r as u8) == winapi::shared::ntdef::TRUE {
        Ok(())
    } else {
        Err(io::Error::last_os_error())
    }
